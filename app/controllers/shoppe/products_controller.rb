module Shoppe
  class ProductsController < Shoppe::ApplicationController
    ...
    def destroy
      ...
      @product.permalink = [@product.permalink, "-", Time.now].join
    ...
    def safe_params
      ...
      params[:product].permit(:name, :color, :color_name, :sku,:position, :permalink, :description, :short_description, :weight, :price, :old_price, :tax_rate_id, :stock_control, :active, :featured, :new_arrivals, :hot_selling, :article_no, :brand, :in_the_box, attachments: [default_image: file_params, extra: file_params], product_attributes_array: [:key, :value, :searchable, :public], product_category_ids: [], size_ids: [])
    end
  end
end
